...
#ifndef REVIEW_STRUCT_DEFINED
#define REVIEW_STRUCT_DEFINED
struct Review
{
    string id;
    string txt;
...
vector<Review> loadDatasetMmap(const string &filePath)
{
    ...
    if (fd == -1)
    {
        ...
        // Do not exit, throw exception for API handling
...
public:
    ...
    // GUI HELPER: Get hash at specific index for visualization
    string getHashAt(int index) {
        if (index >= 0 && index < treeSize) {
            return bytesToHex(tree[index]);
        }
        return "";
    }
